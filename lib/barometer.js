...
function Barometer( opts ) {
  ...
  this.board = Board.mount( this.opts );
  this.firmata = this.board.firmata;

  this.device = DeviceMap[ this.opts.device ];

  // Read event throttling
  this.freq = this.opts.freq || 500;
  ...
  this.interval = setInterval(this.tick.bind(this), this.opts.freq);
}

util.inherits( Barometer, events.EventEmitter );

Barometer.prototype.tick = function(fn) {
  this.device.read( this, this.opts );
  fn && fn();
};
